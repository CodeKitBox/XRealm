...
class RealmProcessor : AbstractProcessor() {
    ...
    override fun process(annotations: Set<TypeElement>, roundEnv: RoundEnvironment): Boolean {
        ...
        if (round == 0) {
            ...
            val columnGenerator = RealmColumnGenerator(processingEnv)
            try {
                columnGenerator.generate()
            } catch (e: IOException) {
                Utils.error("生成RealmColumn 失败"+e.message)
                return ABORT
            }
            val iTableGenerator = RealmTableDaoInterfaceGenerator(processingEnv)
            try {
                iTableGenerator.generate()
            } catch (e: IOException) {
                Utils.error("生成 ITableDao 失败"+e.message)
                return ABORT
            }
        }
    ...
    private fun createProxyClassFiles(typeMirrors: TypeMirrors): Boolean {
        for (metadata in classCollection.classes) {
            ...
            val daoGenerator = RealmDaoGenerator(processingEnv, typeMirrors, metadata, classCollection)
            try {
